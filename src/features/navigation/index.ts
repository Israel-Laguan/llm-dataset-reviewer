...
import { createRowHighlight, highlightCurrentRow } from '../fileHandling/decorations';
...
export const initializeNavigation = (
  ...
  const handleStateChange = (newState: ReviewState) => {
    ...
    const rowResult = getCurrentRow({
      ...
      rows: newState.rows || [] // We'll improve this with proper state management
    });
    
    if (rowResult.ok) {
      updateRowPanel(rowPanelState, rowResult.value);
      highlightCurrentRow(editor, rowResult.value.position, highlightDecoration);
    }
  };
